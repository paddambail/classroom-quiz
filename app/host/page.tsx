...
export default function HostPage() {
  ...
  const loadResults = useCallback(async (id: string) => { const r = await fetch(`/api/quizzes/${id}/results`, { cache: 'no-store' }); if (r.ok) { const data = await r.json(); setRows(data.leaderboard); setAnsweredCount(data.answeredCount ?? 0); } }, []);
  ...
  useEffect(() => {
    ...
    supabase.from('quizzes').select('*').eq('id', id).single().then(({ data }) => {
      if (!data) return;
      setQuiz(data as Quiz); setMode(data.status === 'finished' ? 'finished' : data.status === 'active' ? 'live' : 'ready'); loadQuestion(id); loadResults(id);
    });
    const channel = supabase.channel(`host-${id}`).on('postgres_changes', { event: '*', schema: 'public', table: 'quizzes', filter: `id=eq.${id}` }, (payload) => {
      const next = payload.new as Quiz; setQuiz(next); setMode(next.status === 'finished' ? 'finished' : next.status === 'active' ? 'live' : 'ready'); loadQuestion(id); loadResults(id);
    }).on('postgres_changes', { event: '*', schema: 'public', table: 'students', filter: `quiz_id=eq.${id}` }, () => {
      supabase.from('students').select('*').eq('quiz_id', id).order('joined_at').then(({ data }) => setStudents((data ?? []) as Student[])); loadResults(id);
    }).subscribe();
  ...
  async function action(name: string) {
    if (!quiz || busy) return; setBusy(true); setError('');
